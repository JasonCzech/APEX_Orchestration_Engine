...
2. resolve the engine adapter from handle.connection_id via the connection resolver;
3. ``adapter.abort(handle, reason=...)`` — failures propagate (the operator must know
   the kill did not land) — then best-effort ``teardown``;
4. cancel the thread's pending/running LangGraph runs (tolerates none — the poll loop
...
class EngineRunsRepoLike(Protocol):
    ...
    async def get_latest_for_thread(self, thread_id: str) -> Any: ...
...
class EngineAbortService:
    ...
    async def abort(self, thread_id: str, *, reason: str | None = None) -> EngineAbortResult:
        ...
        try:
            await self._repo.mark_aborted(thread_id)
    ...
    async def _handle_from_projection(self, thread_id: str) -> EngineHandle | None:
        row = await self._repo.get_latest_for_thread(thread_id)
        if row is None:
            return None
        try:
