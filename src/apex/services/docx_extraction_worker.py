"""Resource-limited subprocess entry point for context-document DOCX parsing."""
...
def _apply_resource_limits(*, memory_bytes: int, cpu_seconds: int) -> None:
    """Apply hard child-only limits before importing python-docx/lxml."""
...
def main() -> int:
    if len(sys.argv) != 5:
        _write_error("DOCX extraction worker received invalid arguments")
        return 0
    try:
        ...
        if len(data) > input_limit:
            raise ValueError("DOCX extraction input exceeds its hard limit")

        # Import only after limits are active; python-docx imports lxml.
        from apex.services.text_extraction import _extract_docx_in_worker

        extracted = _extract_docx_in_worker(data, max_chars=max_chars)
