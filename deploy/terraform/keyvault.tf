locals {
  ...
  database_uri      = "postgresql://${var.postgres_admin_login}:${var.postgres_admin_password}@${local.pg_fqdn}:5432/${var.postgres_database}?sslmode=require"
  apex_database_uri = "postgresql+asyncpg://${var.postgres_admin_login}:${var.postgres_admin_password}@${local.pg_fqdn}:5432/${var.postgres_database}?ssl=true"
  redis_uri         = "rediss://:${azurerm_redis_cache.main.primary_access_key}@${azurerm_redis_cache.main.hostname}:${azurerm_redis_cache.main.ssl_port}/0"
}
